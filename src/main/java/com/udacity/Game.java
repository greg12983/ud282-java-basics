...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        int[] sumDiagonal = new int[2]; //[0] backslash diagonal [1] slash diagonal
        int sumMoves = 0;
        ...
        // check sums
        // 3 or -3 means someone won
        // maybe U could fulfill two win conditions at the same time, but still it would be the same person ,e.g. row & column at the same time
        // 9 moves means game is over, but final move can be victorious
        result = checkWinCondition(sumRow, result);
        result = checkWinCondition(sumColumn, result);
        result = checkWinCondition(sumDiagonal, result);
        if (sumMoves == 9 && result.equalsIgnoreCase("None")) result = "Tie";
        return result;
    }

    private String checkWinCondition(int[] sumArray, String result) {
        for (int sum : sumArray) {
            if (sum == 3) result = "X wins";
            if (sum == -3) result = "O wins";
        }
        return result;
    }
